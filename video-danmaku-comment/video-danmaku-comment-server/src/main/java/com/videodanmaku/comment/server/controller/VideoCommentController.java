package com.videodanmaku.comment.server.controller;

import com.alibaba.fastjson.JSON;
import com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper;
import com.baomidou.mybatisplus.core.toolkit.Wrappers;
import com.google.common.base.Preconditions;
import com.videodanmaku.comment.api.common.Result;
...
import com.videodanmaku.comment.server.service.*;
...
public class VideoCommentController {
    ...
    @Resource
    private VideoInfoService videoInfoService;// 这里实际上是视频服务
    ...
    public Result<Boolean> save(@RequestBody SaveVideoCommentReplyReq req) {
        try {
            ...
            Preconditions.checkArgument(Objects.nonNull(req.getVideoId()), "视频ID不能为空！");

            // 这里需要修改为检查视频是否存在
            VideoInfo video = videoInfoService.getById(req.getVideoId());
...
//            Boolean result = shareCommentReplyService.saveComment(req);
            Boolean result = videoCommentReplyService.saveComment(req);
            if (result) {
                if (req.getReplyType() == 1) {
                    // 评论视频
                    shareMessageService.comment(LoginUtil.getLoginId(), video.getCreateBy(),(long) video.getId());
                } else {
                    // 回复评论
                    ...
                    ShareCommentReply reply = videoCommentReplyService.getOne(query);
                    shareMessageService.reply(LoginUtil.getLoginId(), reply.getCreatedBy(),(long) video.getId());
                }
            }
            if (log.isInfoEnabled()) {
    ...
    public Result<List<ShareCommentReplyVO>> list(@RequestBody GetVideoCommentReq req) {
        try {
            if (log.isInfoEnabled()) {
            ...
            List<ShareCommentReplyVO> result = videoCommentReplyService.listComment(req);
            if (log.isInfoEnabled()) {
